...
{
    public class Module : BuildingBlock, IModuleOrInterface, IPortNameSpace, IBuildingBlockWithModuleInstance, IModuleOrInterfaceOrCheckerOrClass, IDocumentRegeion
    {
                ...
                if (parameterOverrides != null)
                {
                    foreach (var vkp in parameterOverrides)
                    {
                        if (module.NamedElements.ContainsKey(vkp.Key) && module.NamedElements[vkp.Key] is DataObjects.Constants.Constants)
                        {
                            ...
                            if (!word.Prototype) param.Defined = true;
